...
const BoardArkScreen = () => {
  ...
  const { data: arkInfo, isLoading: isArkInfoLoading } = useArkInfo();
  ...
  useEffect(() => {
    return navigation.addListener("blur", () => {
      ...
      if (!isSubmitting && !boardResult) {
        reset();
      }
    });
  }, [boardResult, isSubmitting, navigation, reset]);
  ...
  return (
    <NoahSafeAreaView className="flex-1 bg-background">
      <View className="flex-1" testID="board-ark-screen">
